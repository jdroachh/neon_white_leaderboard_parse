...
def format_result_row(display_name, rank, score_ms, total_entries):
    ...
    percentile = (1 - (rank - 1) / total_entries) * 100
    percentile_str = f"top {percentile:.1f}%"
    return {
        "level":      display_name,
        "rank":       rank,
        "time":       time_str,
...
def print_results(rows, player_name, context_label):
    ...
    print(f"{'─'*65}")
    print(f"  {'Level':<28} {'Rank':>7}  {'Time':>10}  {'Percentile'}")
    print(f"{'─'*65}")
    for r in rows:
        print(f"  {r['level']:<28} #{r['rank']:<6}  {r['time']:>10}  {r['percentile']}")
    print(f"{'─'*65}\n")
...
def save_csv(rows, player_name, context_label):
    ...
    with open(filename, "w", newline="", encoding="utf-8") as f:
        writer = csv.DictWriter(
            f, fieldnames=["level", "rank", "time", "percentile", "score_ms", "total"]
        )
        writer.writeheader()
